...
export class BaseRepository<T> {
  ...
  async find(filterQuery?: QueryFilter<T>, options: QueryOptions = {}) {
    const documents = await this.model.find(filterQuery, undefined, options);

    return documents;
  }

  async findOne(filterQuery?: QueryFilter<T>, options?: FindOneOptions) {
  ...
  async count(filterQuery?: QueryFilter<T>) {
  ...
  async findPaginated(
    ...
    const response = new PaginatedResult<HydratedDocument<T>>(
      data,
      totalCount,
      options?.page,
      options?.limit,
